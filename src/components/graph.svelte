<script>
  ...
  const svgWidth = 700;
  const svgHeight = 500;
  const margin = { top: 100, right: 0, bottom: 50, left: 60 };
  let startYear = null;
  let endYear = null;
  ...
  function dragstarted(event, d) {
    let mouseX = event.x;
    dragStartPixel = mouseX;

    svg.select("#drag-rect")
      .attr("x", mouseX)
  }

  function dragged(event, d) {
    let mouseX = event.x;
    let newWidth = Math.abs(mouseX - dragStartPixel);

    if (mouseX < dragStartPixel) {
      svg.select("#drag-rect")
        .attr("x", mouseX);
    }
  ...
  function dragended(event, d) {
    let mouseX = event.x;
    let startYearDrag = xScale.invert(dragStartPixel).getFullYear();
    let currYear = xScale.invert(mouseX).getFullYear();

    if (currYear == startYearDrag) {
      return;
    }

    if (currYear < startYearDrag) {
      let temp = currYear;
      currYear = startYearDrag;
      startYearDrag = temp;
    }

    startYear = startYearDrag;
    endYear = currYear; 
  ...
  svg.append("text")
    ...
    .text("Total Number of Injured");

  svg.append("text")
    .attr("x", svgWidth / 2)
    .attr("y", 20)
    .attr("text-anchor", "middle")
    .text("Number of Injuries per Car Make Over Time");
  ...
  const drawPieChart = (data) => {
    const groupedData = d3.rollup(
        data,
        v => d3.sum(v, d => d.injured + d.killed),
    ...
    const width = 650;
    const height = 650;
    const radius = Math.min(width, height) / 2.7 - 100; // Added margin for labels
